...
-   **Onchain Tools**: Address comparison, EVM address extraction (from various file types), NFT collection management, comparison history, CSV merge with deduplication, duplicate checker, and **Wallet Screener** (batch screen wallets for bots, sybil attacks, airdrop farmers with risk scores, labels, and CSV export - supports Ethereum, Optimism, BSC, Polygon, Base, Arbitrum).
