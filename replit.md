...
**Task Categorization System** (December 2024):
- **Task Types**: Executable (high-priority action items with Rocket icon), Idea (backlog items with Lightbulb icon and rich notes), Milestone (launch targets with Target icon and launch calendar date)
- **Project Tags**: 11 predefined tags - Internal, 4444 Portal, Fireside, Miggles, Titanium, PSX, Signals, RYFT, Tenge, Agency Website, Other
- **Launch Calendar**: Milestone tasks have dedicated launch dates separate from due dates for tracking product launches
- **Rich Notes**: Idea-type tasks support detailed documentation with expandable notes section
- **Visibility Controls**: isPublic toggle (public to team vs private to creator/assignee), editableBy permissions (admin_only, assignee, team)
